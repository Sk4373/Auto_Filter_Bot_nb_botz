...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('❤️‍🔥 Aʙᴏᴜᴛ', callback_data='bot')
                ],[
                    InlineKeyboardButton('💲Eᴀʀɴ Wɪᴛʜ Tʜɪs Bᴏᴛ 💲', callback_data="earn")
                ]]
    ...
    if message.command[1].startswith("reff_"):
        ...
        if user_id == message.from_user.id:
            await message.reply_text("<blockquote>Hᴇʏ Dᴜᴅᴇ, Yᴏᴜ Cᴀɴ'ᴛ Rᴇғᴇʀ Yᴏᴜʀsᴇʟғ 🤣!\n\nsʜᴀʀᴇ ʟɪɴᴋ ʏᴏᴜʀ ғʀɪᴇɴᴅ ᴀɴᴅ ɢᴇᴛ 10 ʀᴇғᴇʀʀᴀʟ ᴘᴏɪɴᴛ ɪғ ʏᴏᴜ ᴀʀᴇ ᴄᴏʟʟᴇᴄᴛɪɴɢ 100 ʀᴇғᴇʀʀᴀʟ ᴘᴏɪɴᴛs ᴛʜᴇɴ ʏᴏᴜ ᴄᴀɴ ɢᴇᴛ 1 ᴍᴏɴᴛʜ ғʀᴇᴇ ᴘʀᴇᴍɪᴜᴍ ᴍᴇᴍʙᴇʀsʜɪᴘ.</blockquote>")
            return
        if referdb.is_user_in_list(message.from_user.id):
            await message.reply_text("Yᴏᴜ Aʀᴇ ᴀʟʀᴇᴀᴅʏ ɪɴᴠɪᴛᴇᴅ ❗")
            return
        try:
        ...
        if settings.get('fsub_id', AUTH_CHANNEL) == AUTH_REQ_CHANNEL:
            if AUTH_REQ_CHANNEL and not await is_req_subscribed(client, message):
                try:
                    ...
                    return
                btn = [[
                    InlineKeyboardButton("⚠️ Jᴏɪɴ Hᴇʀᴇ ⚠️", url=invite_link.invite_link)
                ]]
            ...
            if channel != AUTH_CHANNEL and not await is_subscribed(client, message.from_user.id, channel):
                ...
                btn.append([InlineKeyboardButton("⚠️ Jᴏɪɴ Hᴇʀᴇ ⚠️", url=invite_link_custom.invite_link)])
            ...
            if not await is_req_subscribed(client, message):
                ...
                btn.append([InlineKeyboardButton("⚠️ Jᴏɪɴ Hᴇʀᴇ ⚠️", url=invite_link_default.invite_link)])
    ...
    ibmp = message.command[1] # @infinity_botzz telegram
    try:
        btn = await infinitymp_subscribed(client, message, AUTH_CHANNELS)
        if btn:
            btn.append(
                [InlineKeyboardButton("🤭 Tʀʏ Aɢᴀɪɴ 🤭", callback_data=f"checksub#{ibmp}")]
            )
            reply_markup = InlineKeyboardMarkup(btn)
            await client.send_photo(
            chat_id=message.from_user.id,
                    photo=random.choice(FSUB_IMG),
                    caption=script.FORCESUBS_TEXT,
                    reply_markup=InlineKeyboardMarkup(btn),
                    parse_mode=enums.ParseMode.HTML,
                    reply_to_message_id=message.id
                )
        ...
        await log_error(client, f"Got Error In Force Subscription Funtion by ib.\n\n Error - {n}")
        print(f"Error In Fsub 2 :- {n}")
    ...
    if not await db.has_premium_access(user_id):
        try:
            ...
            if settings.get("is_verify", IS_VERIFY) and (not user_verified or is_second_shortener or is_third_shortener):                
                ...
                buttons = [[
                    InlineKeyboardButton(text="❗Vᴇʀɪғʏ❗", url=verify)
                ],[
                    InlineKeyboardButton(text="⁉️ Hᴏᴡ Tᴏ Vᴇʀɪꜰʏ ⁉️", url=howtodownload)
                ]]
                reply_markup=InlineKeyboardMarkup(buttons)
                if await db.user_verified(user_id): 
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            if STREAM_MODE:
                btn = [
                    [InlineKeyboardButton('🚀 Fᴀꜱᴛ ᴅᴏᴡɴʟᴏᴀᴅ / Sᴛʀᴇᴀᴍ Oɴʟɪɴᴇ 🖥️', callback_data=f'streamfile:{file_id}')],
                    [InlineKeyboardButton('📌 Jᴏɪɴ Uᴘᴅᴀᴛᴇꜱ Cʜᴀɴɴᴇʟ 📌', url=UPDATE_CHANNEL_LNK)]
                ]
            else:
                btn = [
                    [InlineKeyboardButton('📌 Jᴏɪɴ Uᴘᴅᴀᴛᴇꜱ Cʜᴀɴɴᴇʟ 📌', url=UPDATE_CHANNEL_LNK)]
        ...
        for file in files:
            ...
            if STREAM_MODE:
                btn = [
                    [InlineKeyboardButton('🚀 Fᴀꜱᴛ Dᴏᴡɴʟᴏᴀᴅ / Sᴛʀᴇᴀᴍ Oɴʟɪɴᴇ 🖥️', callback_data=f'streamfile:{file_id}')],
                    [InlineKeyboardButton('📌 Jᴏɪɴ Uᴘᴅᴀᴛᴇꜱ Cʜᴀɴɴᴇʟ 📌', url=UPDATE_CHANNEL_LNK)]  
                ]
            else:
                btn = [
...
async def trendlist(client, message):
    ...
    formatted_list += f"\n\n{additional_message}"
    reply_text = f"<b>Top {len(truncated_messages)} Tʀᴇɴᴅɪɴɢ ᴏғ ᴛʜᴇ ᴅᴀʏ 👇:</b>\n\n{formatted_list}"
...
async def stop_button(bot, message):
    msg = await bot.send_message(text="<b><i>Rᴇsᴛᴀʀᴛɪɴɢ...</i></b>", chat_id=message.chat.id)       
    await asyncio.sleep(3)
    await msg.edit("<b><i><u>Rᴇꜱᴛᴀʀᴛᴇᴅ</u> ✅</i></b>")
...
async def set_tutorial_1(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>ᴄᴏᴍᴍᴀɴᴅ ɪɴᴄᴏᴍᴘʟᴇᴛᴇ !!\n\nᴜꜱᴇ ʟɪᴋᴇ ᴛʜɪꜱ -</b>\n\n<code>/set_tutorial https://t.me/example</code>")
...
async def set_tutorial_2(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>ᴄᴏᴍᴍᴀɴᴅ ɪɴᴄᴏᴍᴘʟᴇᴛᴇ !!\n\nᴜꜱᴇ ʟɪᴋᴇ ᴛʜɪꜱ -</b>\n\n<code>/set_tutorial_2 https://t.me/example/2</code>")
...
async def set_shortner(c, m):
    ...
    if len(m.text.split()) == 1:
        await m.reply("<b>ᴜꜱᴇ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ ʟɪᴋᴇ ᴛʜɪꜱ - \n\n`/set_shortner omegalinks.in a7ac9b3012c67d7491414cf272d82593c75cbb`</b>")
        return        
...
async def set_shortner_2(c, m):
    ...
    if not await is_check_admin(c, grp_id, m.from_user.id):
        return await m.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    if len(m.text.split()) == 1:
        await m.reply("<b>ᴜꜱᴇ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ ʟɪᴋᴇ ᴛʜɪꜱ - \n\n`/set_shortner_2 omegalinks.in a7ac9b3012c67d7491414cf272d823c75f6cbb`</b>")
        return
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await asyncio.sleep(1.2)
    await sts.delete()
...
async def set_shortner_3(c, m):
    ...
    if not await is_check_admin(c, grp_id, m.from_user.id):
        return await m.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    if len(m.text.split()) == 1:
        await m.reply("<b>ᴜꜱᴇ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ ʟɪᴋᴇ ᴛʜɪꜱ - \n\n`/set_shortner_3 omegalinks.in a7ac9b3012c67d7491414cf272d82593cf6cbb`</b>")
        return
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await asyncio.sleep(1.2)
    await sts.delete()
    ...
    try:
        ...
        await save_group_settings(grp_id, 'shortner_three', SHORTENER_WEBSITE3)
        await save_group_settings(grp_id, 'api_three', SHORTENER_API3)
        await m.reply_text(f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/CpFlicks_Support>sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/set_shortner_3 mdiskshortner.link e7beb3c8f756dfa15d0bec495abc658c0dfa95`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>", quote=True)        
